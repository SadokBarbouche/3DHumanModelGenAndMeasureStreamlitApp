...
class Pose_Detection_Toolkit:        
    ...
    def angle_between_vectors(self, vector1, vector2):
        ...
        return np.arccos(np.dot(vector1, vector2) / (norm_A * norm_B)) * 180/ np.pi # Angle in deg not in rad 
        # arcos : [-1,1] --> [0,pi]
    ...
    def generate_vectors(self,results,image_height, image_width):
            ...
            shoulder_elbow_left_x = landmarks.landmark[mph_landmarks.LEFT_ELBOW].x - landmarks.landmark[mph_landmarks.LEFT_SHOULDER].x
            shoulder_elbow_right_x = landmarks.landmark[mph_landmarks.RIGHT_ELBOW].x - landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].x
            shoulder_elbow_left_y = landmarks.landmark[mph_landmarks.LEFT_ELBOW].y - landmarks.landmark[mph_landmarks.LEFT_SHOULDER].y
            shoulder_elbow_right_y = landmarks.landmark[mph_landmarks.RIGHT_ELBOW].y - landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].y

            elbow_wrist_left_y = landmarks.landmark[mph_landmarks.LEFT_WRIST].y - landmarks.landmark[mph_landmarks.LEFT_ELBOW].y
            elbow_wrist_right_y = landmarks.landmark[mph_landmarks.RIGHT_WRIST].y - landmarks.landmark[mph_landmarks.RIGHT_ELBOW].y
            elbow_wrist_left_x = landmarks.landmark[mph_landmarks.LEFT_WRIST].x - landmarks.landmark[mph_landmarks.LEFT_ELBOW].x
            elbow_wrist_right_x = landmarks.landmark[mph_landmarks.RIGHT_WRIST].x - landmarks.landmark[mph_landmarks.RIGHT_ELBOW].x

            wrist_index_left_x = landmarks.landmark[mph_landmarks.LEFT_WRIST].x - landmarks.landmark[mph_landmarks.LEFT_INDEX].x
            wrist_index_right_x = landmarks.landmark[mph_landmarks.RIGHT_WRIST].x - landmarks.landmark[mph_landmarks.RIGHT_INDEX].x
            wrist_index_left_y = landmarks.landmark[mph_landmarks.LEFT_WRIST].y - landmarks.landmark[mph_landmarks.LEFT_INDEX].y
            wrist_index_right_y = landmarks.landmark[mph_landmarks.RIGHT_WRIST].y - landmarks.landmark[mph_landmarks.RIGHT_INDEX].y

            shoulder_hip_left_x = landmarks.landmark[mph_landmarks.LEFT_SHOULDER].x - landmarks.landmark[mph_landmarks.LEFT_HIP].x
            shoulder_hip_right_x = landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].x - landmarks.landmark[mph_landmarks.RIGHT_HIP].x
            shoulder_hip_left_y = landmarks.landmark[mph_landmarks.LEFT_SHOULDER].y - landmarks.landmark[mph_landmarks.LEFT_HIP].y
            shoulder_hip_right_y = landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].y - landmarks.landmark[mph_landmarks.RIGHT_HIP].y

            hip_knee_left_x = landmarks.landmark[mph_landmarks.LEFT_HIP].x - landmarks.landmark[mph_landmarks.LEFT_KNEE].x
            hip_knee_left_y = landmarks.landmark[mph_landmarks.LEFT_HIP].y - landmarks.landmark[mph_landmarks.LEFT_KNEE].y
            hip_knee_right_x = landmarks.landmark[mph_landmarks.RIGHT_HIP].x - landmarks.landmark[mph_landmarks.RIGHT_KNEE].x
            hip_knee_right_y = landmarks.landmark[mph_landmarks.RIGHT_HIP].y - landmarks.landmark[mph_landmarks.RIGHT_KNEE].y

            knee_ankle_left_x = landmarks.landmark[mph_landmarks.LEFT_ANKLE].x - landmarks.landmark[mph_landmarks.LEFT_KNEE].x
            knee_ankle_left_y = landmarks.landmark[mph_landmarks.LEFT_ANKLE].y - landmarks.landmark[mph_landmarks.LEFT_KNEE].y
            knee_ankle_right_x = landmarks.landmark[mph_landmarks.RIGHT_ANKLE].x - landmarks.landmark[mph_landmarks.RIGHT_KNEE].x
            knee_ankle_right_y = landmarks.landmark[mph_landmarks.RIGHT_ANKLE].y - landmarks.landmark[mph_landmarks.RIGHT_KNEE].y

            footindex_ankle_left_x = landmarks.landmark[mph_landmarks.LEFT_ANKLE].x - landmarks.landmark[mph_landmarks.LEFT_FOOT_INDEX].x
            footindex_ankle_left_y = landmarks.landmark[mph_landmarks.LEFT_ANKLE].y - landmarks.landmark[mph_landmarks.LEFT_FOOT_INDEX].y
            footindex_ankle_right_x = landmarks.landmark[mph_landmarks.RIGHT_ANKLE].x - landmarks.landmark[mph_landmarks.RIGHT_FOOT_INDEX].x
            footindex_ankle_right_y = landmarks.landmark[mph_landmarks.RIGHT_ANKLE].y - landmarks.landmark[mph_landmarks.RIGHT_FOOT_INDEX].y

            right_shoulder_left_shoulder_x = landmarks.landmark[mph_landmarks.LEFT_SHOULDER].x - landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].x
            right_shoulder_left_shoulder_y = landmarks.landmark[mph_landmarks.LEFT_SHOULDER].y - landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].y
            der_left_shoulder_y = results.pose_landmarks.landmark[mph_landmarks.LEFT_SHOULDER].y - results.pose_landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].y
            
            footindex_heel_left_x = landmarks.landmark[mph_landmarks.LEFT_FOOT_INDEX].x - landmarks.landmark[mph_landmarks.LEFT_HEEL].x
            footindex_heel_left_y = landmarks.landmark[mph_landmarks.LEFT_FOOT_INDEX].y - landmarks.landmark[mph_landmarks.LEFT_HEEL].y
            
            footindex_heel_right_x = landmarks.landmark[mph_landmarks.RIGHT_FOOT_INDEX].x - landmarks.landmark[mph_landmarks.RIGHT_HEEL].x
            footindex_heel_right_y = landmarks.landmark[mph_landmarks.RIGHT_FOOT_INDEX].y - landmarks.landmark[mph_landmarks.RIGHT_HEEL].y
    ...
    def Pose_Dectection(self,vectors): # Need to find a way to get a more accurate angle range
        # The needed angles are the following : 
        ##  Between the shoulder_elbow vector and the shoulder_hip vector
        angle_se_sh_left = self.angle_between_vectors(vectors['shoulder_elbow'][0],-vectors['shoulder_hip'][0]) 
        angle_se_sh_right = self.angle_between_vectors(vectors['shoulder_elbow'][1],-vectors['shoulder_hip'][1]) 
        ##  Between the hip_knee vector and the shoulder_hip vector
        angle_hk_sh_left = self.angle_between_vectors(vectors['hip_knee'][0],-vectors['shoulder_hip'][0]) 
        angle_hk_sh_right = self.angle_between_vectors(vectors['hip_knee'][1],-vectors['shoulder_hip'][1]) 
        ##  Between the elbow_wrist vector and the shoulder_elbow vector
        angle_ew_se_left = self.angle_between_vectors(vectors['elbow_wrist'][0],-vectors['shoulder_elbow'][0]) 
        angle_ew_se_right = self.angle_between_vectors(vectors['elbow_wrist'][1],-vectors['shoulder_elbow'][1]) 
        ##  Between the hip_knee vector and the knee_ankle vector
        angle_hk_ka_left = self.angle_between_vectors(vectors['hip_knee'][0],vectors['knee_ankle'][0]) 
        angle_hk_ka_right = self.angle_between_vectors(vectors['hip_knee'][1],vectors['knee_ankle'][1]) 
    ...
    def image_orientation(self,results,vectors): # Checks whether the body in our image is turning to the left or the right
        landmarks = results.pose_landmarks
        mph_landmarks = mp_holistic.PoseLandmark
    
        right_shoulder_left_shoulder_x = landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].x - landmarks.landmark[mph_landmarks.LEFT_SHOULDER].x
        right_shoulder_left_shoulder_y = landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].y - landmarks.landmark[mph_landmarks.LEFT_SHOULDER].y
        right_shoulder_left_shoulder_z = landmarks.landmark[mph_landmarks.RIGHT_SHOULDER].z - landmarks.landmark[mph_landmarks.LEFT_SHOULDER].z
        right_shoulder_left_shoulder = np.array([right_shoulder_left_shoulder_x,right_shoulder_left_shoulder_y,right_shoulder_left_shoulder_z])
        
        angle = self.angle_between_vectors(right_shoulder_left_shoulder,np.array([1,0,0]))
        
        hand_threshold = 0.10898104310035706 # This value is set by observation of some examples of correct and icorrect poses and can/should be corrected
